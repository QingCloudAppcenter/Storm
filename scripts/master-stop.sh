...
for pid in $(ps ax | grep -i 'storm.ui.core'  | grep -v grep | awk '{print $1}')
do
kill -s TERM  $pid
done
 for i in $(seq 0 30); do
     if ! ps -ef | grep storm.daemon |grep -v grep > /dev/null; then
         echo "storm  is successfully terminated"
         exit 0
     fi
     sleep 1
done
ps -ef|grep storm.ui.core|grep -v grep|cut -c 9-15|xargs kill -9
ps -ef|grep storm.daemon|grep -v grep|cut -c 9-15|xargs kill -9
if [ $? -eq 0 ]; then
      echo "storm master node is successfully killed"
      exit 0
  else
